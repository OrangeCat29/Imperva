...
def run():
    ...
    m = re.findall('(total number of requests.*)', output[0].decode('utf-8').lower())
...
def getDiskStats():
    pipe = Popen(['cat','/proc/mounts'], stdout=PIPE)
    output = pipe.communicate()
    mountsAry = str(output[0], 'utf-8').split("\n")

    for mount in mountsAry:
        if mount.strip() != "":
            mountAry = mount.replace(",", " ").split(" ")
            if len(mountAry) > 1 and mountAry[1][:1] == "/":
                pipe = Popen(['df', mountAry[1]], stdout=PIPE)
                output = pipe.communicate()
                mountStats = str(output[0], 'utf-8').split("\n")

                if len(mountStats) > 1:
                    mountStats.pop(0)

                mountStatsAry = ' '.join(mountStats).replace("\n", " ").split()
                print(f"DEBUG: mountStatsAry = {mountStatsAry}")  # Debug để kiểm tra dữ liệu

                if len(mountStatsAry) > 3:
                    influxDbStats["imperva_gw_disk"]["volume=" + mountAry[1]] = []
                    influxIfaceStatAry = influxDbStats["imperva_gw_disk"]["volume=" + mountAry[1]]
                    influxIfaceStatAry.append("disk_capacity=" + mountStatsAry[1])
                    influxIfaceStatAry.append("disk_used=" + mountStatsAry[2])
                    ...
                    print(f"⚠️ WARNING: mountStatsAry không có đủ phần tử! Dữ liệu lỗi: {mountStatsAry}")
...
def getSysStats():
    with open('/opt/SecureSphere/etc/bootstrap.xml', 'r') as content_file:
        ...
        output = pipe.communicate()
        for stat in output[0].decode('utf-8').split("\n"):
            if stat.strip()!="":
                statAry = stat.split(" ")
                key = statAry.pop(0)
                val = statAry.pop()
...
gwSizingStats = {
    ...
    # Physical Appliances 30 series
    "X10K3":{"gw_supported_kbps":"10000000","gw_supported_hps":"72000"},
    "X2530":{"gw_supported_kbps":"500000","gw_supported_hps":"5000"},
    "X4530":{"gw_supported_kbps":"1000000","gw_supported_hps":"9000"},
    "X6530":{"gw_supported_kbps":"2000000","gw_supported_hps":"18000"},
    "X8530":{"gw_supported_kbps":"5000000","gw_supported_hps":"36000"},
    # Virtual Appliances
